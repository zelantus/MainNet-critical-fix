...
//        ui->optInRBF->setCheckState(model->getDefaultWalletRbf() ? Qt::Checked : Qt::Unchecked);
        ...
        ui->reissueCostLabel->setText(tr("Cost") + ": " + ZelantusUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), GetBurnAmount(AssetType::REISSUE)));
...
void ReissueAssetDialog::CheckFormState()
{
    ...
    if (!ui->addressText->text().isEmpty()) {
        if (!IsValidDestination(dest)) {
            showMessage(tr("Invalid Zelantus Destination Address"));
            return;
        }
    }
    ...
    if (fReissuingRestricted) {
        ...
        if (!strVerifier.empty()) {
            ...
            if (fHasQuantity && strAddress.empty()) {
                ...
                showMessage(tr("Warning: Restricted Token Issuance requires an address"));
                return;
            }
...
void ReissueAssetDialog::onReissueAssetClicked()
{
    ...
    QString amount = "<b>" + QString::fromStdString(ValueFromAmountString(GetReissueAssetBurnAmount(), 8)) + " ZELS";
    amount.append("</b>");
