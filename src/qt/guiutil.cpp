...
#include "zelantusaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Zelantus address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new ZelantusAddressEntryValidator(parent));
    widget->setCheckValidator(new ZelantusAddressCheckValidator(parent));
}
...
bool parseZelantusURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no zelantus: URI
    if(!uri.isValid() || uri.scheme() != QString("zelantus"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!ZelantusUnits::parse(ZelantusUnits::ZELS, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseZelantusURI(QString uri, SendCoinsRecipient *out)
{
    // Convert zelantus:// to zelantus:
    //
    //    Cannot handle this later, because zelantus:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("zelantus://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "zelantus:");
    }
    QUrl uriInstance(uri);
    return parseZelantusURI(uriInstance, out);
}

QString formatZelantusURI(const SendCoinsRecipient &info)
{
    QString ret = QString("zelantus:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(ZelantusUnits::format(ZelantusUnits::ZELS, info.amount, false, ZelantusUnits::separatorNever));
        paramCount++;
    }
...
bool openZelantusConf()
{
    ...
    /* Open zelantus.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Zelantus.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Zelantus (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Zelantus (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Zelantus*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "zelantus.desktop";
    return GetAutostartDir() / strprintf("zelantus-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a zelantus.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Zelantus\n";
        else
            optionFile << strprintf("Name=Zelantus (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the zelantus app
...
bool GetStartOnSystemStartup()
{
    CFURLRef zelantusAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (zelantusAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, zelantusAppUrl);

    CFRelease(zelantusAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add zelantus app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, zelantusAppUrl, nullptr, nullptr);
    }
