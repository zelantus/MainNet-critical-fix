...
#include "zelantusgui.h"

#include "zelantusunits.h"
...
const std::string ZelantusGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString ZelantusGUI::DEFAULT_WALLET = "~Default";

ZelantusGUI::ZelantusGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void ZelantusGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Zelantus address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and zelantus: URIs)"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Zelantus addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Zelantus addresses"));
    ...
    openAction->setStatusTip(tr("Open a zelantus: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Zelantus command-line options").arg(tr(PACKAGE_NAME)));
...
void ZelantusGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void ZelantusGUI::createToolBars()
{
    if(walletFrame)
    {
        /** ZELS START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/zelantustext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* zelantusLabelLayout = new QVBoxLayout(toolbarWidget);
        zelantusLabelLayout->addWidget(label);
        zelantusLabelLayout->addWidget(toolbar);
        zelantusLabelLayout->setDirection(QBoxLayout::TopToBottom);
        zelantusLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Zelantus Market Price"));
        ...
        labelVersionUpdate->setText("<a href=\"https://github.com/ZelantusProject/Zelantus/releases\">New Wallet Version Available</a>");
        ...
        // Get the latest Zelantus release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/ZelantusProject/Zelantus/releases";
...
void ZelantusGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool ZelantusGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool ZelantusGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void ZelantusGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void ZelantusGUI::setWalletActionsEnabled(bool enabled)
{
...
void ZelantusGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void ZelantusGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void ZelantusGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void ZelantusGUI::optionsClicked()
{
...
void ZelantusGUI::showDebugWindow()
{
...
void ZelantusGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void ZelantusGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void ZelantusGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void ZelantusGUI::openClicked()
{
...
void ZelantusGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void ZelantusGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void ZelantusGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void ZelantusGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void ZelantusGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void ZelantusGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** ZELS START */
void ZelantusGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void ZelantusGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void ZelantusGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void ZelantusGUI::gotoRestrictedAssetsPage()
{
...
void ZelantusGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Zelantus network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void ZelantusGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void ZelantusGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void ZelantusGUI::updateHeadersSyncProgressLabel()
{
...
void ZelantusGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void ZelantusGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Zelantus"); // default title
...
void ZelantusGUI::changeEvent(QEvent *e)
{
...
void ZelantusGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ZelantusGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void ZelantusGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "ZELS")
        msg += tr("Amount: %1\n").arg(ZelantusUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(ZelantusUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void ZelantusGUI::checkAssets()
{
    ...
    if (AreRestrictedAssetsDeployed()) {
        ...
        restrictedAssetAction->setToolTip(tr("Restricted Token not yet active"));
    }
}
#endif // ENABLE_WALLET

void ZelantusGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ZelantusGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ZelantusGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ZelantusGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ZelantusGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void ZelantusGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void ZelantusGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ZelantusGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ZelantusGUI::detectShutdown()
{
...
void ZelantusGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void ZelantusGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void ZelantusGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(ZelantusGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(ZelantusGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void ZelantusGUI::subscribeToCoreSignals()
{
...
void ZelantusGUI::unsubscribeFromCoreSignals()
{
...
void ZelantusGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void ZelantusGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<ZelantusUnits::Unit> units = ZelantusUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const ZelantusUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(ZelantusUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(ZelantusUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (ZelantusUnits::Unit u : ZelantusUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(ZelantusUnits::name(u)), this);
...
{
    setText(ZelantusUnits::name(newUnits));
}
...
void ZelantusGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=ZELSBTC"));
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void ZelantusGUI::mnemonic()
{
...
void ZelantusGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/ZelantusProject/Zelantus/releases"));
