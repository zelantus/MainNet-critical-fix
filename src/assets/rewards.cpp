...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for ZELS
    ...
    if (p_rewardSnapshot.strDistributionAsset != "ZELS") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is ZELS with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using ZELS differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "ZELS") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Zelantus address (already validated during ownership snapshot creation)
