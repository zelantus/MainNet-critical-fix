...
public :
    ...
    //! Restricted Token Verifier Caches
...
bool CheckIssueDataTx(const CTxOut& txOut); // OP_RAVEN_ASSET ZELSQ (That is a Q as in Que not an O)
bool CheckOwnerDataTx(const CTxOut& txOut);// OP_RAVEN_ASSET ZELSO
bool CheckReissueDataTx(const CTxOut& txOut);// OP_RAVEN_ASSET ZELSR
bool CheckTransferOwnerTx(const CTxOut& txOut);// OP_RAVEN_ASSET ZELST
