...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Zelantus database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Zelantus datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Zelantus datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Zelantus 0.7 datadir."
    ;;
  3)
    echo "Detected Zelantus pre-0.8 datadir."
    ;;
  4)
    echo "Detected Zelantus 0.8 datadir."
    ;;
esac
