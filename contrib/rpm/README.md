...
The RPM spec file provided here is for Zelantus-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://zelantus.com/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Zelantus git repository](https://github.com/ZelantusProject/Zelantus) but are not part of
...
Currently only `Source10` is used, for the example `zelantus.conf` file.
...
in the zelantus git repository. At present only an SVG version of the Zelantus
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the zelantus
...
needed when 0.12.1 is released, a proper fix is already in the Zelantus git
master branch.
...
idea to build Zelantus Core against a static version of that library compiled
...
The `zelantus.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `zelantus-server` package. At this point, most RPM based
...
The `zelantus-server` package should have SELinux support. How to properly do
...
Zelantus-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/zelantus/). He has been
packaging Zelantus for Fedora at least since 2012.
...
4. I renamed the `zelantus` package that contains the Qt GUI to `zelantus-core` as
that appears to be how the general population refers to it, in contrast to
`zelantus-xt` or `zelantus-classic`. I wanted to make sure the general population
