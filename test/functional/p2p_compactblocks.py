...
# TestNode: A peer we use to send messages to zelantusd, and store responses.
...
class CompactBlocksTest(ZelantusTestFramework):
    ...
    # This test actually causes zelantusd to (reasonably!) disconnect us, so do this last.
    ...
    # Test that zelantusd requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, node, test_node, version):
        ...
        # Note that it's possible for zelantusd to be smart enough to know we're
    ...
    def test_getblocktxn_handler(node, test_node, version):
        # zelantusd will not send blocktxn responses for blocks whose height is
